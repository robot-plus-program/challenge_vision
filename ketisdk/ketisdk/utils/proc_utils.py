...
class ProcUtils():
    ...
    def init_vid_writer(self, size, fold=None, name=None, fps=15, isColor=True):
        if name is None: name = self.get_current_time_str() + '.mp4'
...
class Regression():
    def fitting_surface(self, X,Y, Z):
        ...
        A = np.array([[var_X, cov_XY], [cov_XY, var_Y]], dtype='float32')
        bb = np.array([[cov_ZX], [cov_YZ]], dtype='float32')
...
class WorkSpace():
    ...
    def get_mask(self, max_shape=None):
        return PolyUtils().poly2mask(poly=self.pts, max_shape=max_shape)

    def get_mask_crop(self):
        mask = self.get_mask()
        return mask[self.top:self.bottom, self.left:self.right]
...
class CFG(BasDataObj):
    ...
    def merge_with(self, args, force=False):
        for section_name in args.keys():
            ...
            for option_name in section_value.keys():
                if hasattr(section_, option_name) and not force: continue
                section_.__setattr__(option_name, section_value.__getattribute__(option_name))


    def write(self, path=None):
        if path is None: path = 'args.cfg'
    ...
    def logical_and(self, acfg):
        cfg = self.copy()
        for section_name in self.keys():
            if not hasattr(acfg, section_name):
                cfg.__delattr__(section_name)
                continue
            section = self.__getattribute__(section_name)
            asection = acfg.__getattribute__(section_name)
            for option_name in section.keys():
                if not hasattr(asection, option_name):
                    section.__delattr__(option_name)
                # value = section.__getattribute__(option_name)
                # section.__setattr__(option_name, asection.__getattribute__(option_name))
            cfg.__setattr__(section_name, section)
...
class Timer():
    ...
    def get_pinned_times(self):
        return [self.times[j+1]-t for j,t in enumerate(self.times[:-1])]

    def get_pinned_time_dict(self):
        pinned_times = dict()
        for j,t in enumerate(self.times[:-1]):
            dt = self.times[j+1]-t
            pinned_times.update({self.labels[j]: dt})
        pinned_times.update({'total': self.run_time()})
        return pinned_times

    def get_weight_average_dict(self, timer_dicts, weights=None):
        num_timer = len(timer_dicts)
        if weights is None: weights = [1]*num_timer
        weights = np.array(weights)/np.sum(weights)
        average_time = dict()
        for lb in timer_dicts[0]:
            lb_times = np.array([td[lb] for td in timer_dicts])
            avg_lb_time = np.sum(np.multiply(lb_times, weights))
            average_time.update({lb: avg_lb_time})
        return average_time

class ArrayUtils():
    def normalize(self, anArray, denoise=False):
        if denoise: anArray = cv2.blur(anArray, (3,3))
        amin, amax = np.amin(anArray), np.amax(anArray)
        return (anArray-amin)/(amax-amin)

    def rotate2d(self, Input, Theta, org=(0,0)):
        '''

        Args:
            Input: 2xnum_point
            Theta: dergee
            org: rotating coordinate
        ...
        Input_ = np.copy(Input).astype('float32')
        Input_[0,:] -=  org[0]
        Input_[1,:] -=  org[1]

        Output = np.dot(R, Input_)
        Output[0,:] +=  org[0]
        Output[1, :] += org[1]

        return Output.astype('int')
    def rotateBox(self, Boxes, Theta, org=(0,0)):
        '''
        ...
        Input_ = np.copy(Boxes).astype('float32')
        Input_[[0,2], :] -= org[0]
        Input_[[1,3], :] -= org[1]

        Output1 = np.dot(R, Input_[[0,1],:])
        Output2 = np.dot(R, Input_[[0,3],:])
        Output3 = np.dot(R, Input_[[2,3],:])
        Output4 = np.dot(R, Input_[[2,1],:])
    ...
    def get_gradient(self, im_in, r=1, axis=0, nb_r=None):
        ...
        # cv2.imwrite('data/grad.png', (10*np.abs(out)).astype('uint8'))

        if nb_r is not None:
            ss =  2*nb_r+1
            kernel = np.ones((ss, ss), 'float32')/(ss*ss-1)
            kernel[(nb_r, nb_r)] = 0
            nb_grad = cv2.filter2D(out, ddepth=-1, kernel=kernel)
            if axis==0:
                out[r:-r, :] -= nb_grad[r:-r, :]
            else:
                out[:, r:-r] -= nb_grad[:, r:-r]

        # cv2.imwrite('data/grad_nb.png', (10 * np.abs(out)).astype('uint8'))
    ...
    def linear_regression(self,X, Y, eps=0.00001):
...
torch_arctan2 = lambda y,x: torch.arctan(torch.divide(y,x+1e-10)) + (x<0) * torch.pi * torch.sign(y)

def argwhere_sampling(mask, deg=5, stride=3):
    use_torch = isinstance(mask, torch.Tensor)
    X = np.argwhere(mask>0)[:, ::-1].astype('float32') if not use_torch else torch.nonzero(mask>0)[:,[1,0]].type(torch.float32)
    Xc = np.mean(X, axis=0, keepdims=True) if not use_torch else torch.mean(X, dim=0, keepdim=True)
    X -= Xc
    Rho = np.linalg.norm(X, axis=1, keepdims=True) if not use_torch else torch.norm(X, dim=1)
    Phi = np.arctan2(X[:, [1]], X[:,[0]]) if not use_torch else torch_arctan2(X[:, [1]], X[:,[0]])
    Phi = Phi * 180 / np.pi + 180

    loc = (np.where if not use_torch else torch.where)(Phi % deg < 1)[0]
    Rho, Phi = Rho[loc].reshape(-1, 1), Phi[loc].reshape(-1, 1)

    loc = (np.where if not use_torch else torch.where)(Rho % stride < 1)[0]
    Rho, Phi = Rho[loc].reshape(-1, 1), Phi[loc].reshape(-1, 1)

    Phi = (Phi- 180) * np.pi / 180
    cosp= (np.cos if not use_torch else torch.cos)(Phi)
    sinp = (np.sin if not use_torch else torch.sin)(Phi)
    D =  np.concatenate((cosp, sinp), axis=-1) if not use_torch else torch.cat((cosp, sinp), dim=-1)
    X = Xc + (np.multiply if not use_torch else torch.multiply)(Rho, D)

    return (X[:, 1].astype('int'), X[:,0].astype('int')) if not use_torch else (X[:,1].type(torch.int64), X[:,0].type(torch.int64))
